...
func (e *Edit) CreateHandle() (win32.HWND, error) {
	...
	return win32util.CreateWindow(&win32util.Wnd{
		...
		X:          metrics.ToPx(e.Spec.X, dpi).Value(),
		Y:          metrics.ToPx(e.Spec.Y, dpi).Value(),
		Width:      metrics.ToPx(e.Spec.Width, dpi).Value(),
		Height:     metrics.ToPx(e.Spec.Height, dpi).Value(),
